(function() {
  ...
  if (simpleSyS.app == null) {
    simpleSyS.app = {
      ViewLoaderC: function() {
        return void 0;
      },
      logining: {
  ...
  isc.RPCManager.addClassProperties({
    "handleError": function(response, request) {
      var codeName, data, delta, extraText, message, messageToShow, opName, ref, ref1, ref2;
      if (response.status === isc.RPCResponse.STATUS_LOGIN_REQUIRED) {
        if (isc.RPCManager.loginRequired != null) {
          isc.RPCManager.loginRequired(response, request);
        } else if (isc.RPCManagerSS.loginRequired != null) {
          isc.RPCManagerSS.loginRequired(response, request);
        }
      } else {
        if (((ref = response.data) != null ? ref.data : void 0) != null) {
          response.data = response.data.data;
        }
        if ((response.data != null) && isc.isA.String(response.data) === true) {
          message = response.data;
          if (isc.isA.String(message) === true) {
            messageToShow = message;
            if (messageToShow.length > this.maxErrorMessageLength) {
              delta = messageToShow.length - this.maxErrorMessageLength;
              messageToShow = messageToShow.substring(0, this.maxErrorMessageLength) + ("<br><br>...(" + delta + " bytes truncated - set isc.RPCManager.maxErrorMessageLength > " + this.maxErrorMessageLength + " to see more or check the Developer Console for full error)...");
            }
          }

          /*isc.error messageToShow, "958c9eec-999a-92a1-b3a7-2877a657ec55" */
        } else {
          if (((ref1 = response.errorStruct) != null ? ref1.error : void 0) != null) {
            response.data.error = response.errorStruct.error;
          }
          if (((ref2 = response.data) != null ? ref2.error : void 0) != null) {
            if (isc.isA.Array(response.data.error) === true) {
              isc.error("Ошибка: " + response.data.error.length + " элемента.");
            } else if (response.data.error.message !== strEmpty) {
            ...
            if (isc.isA.String(codeName) === true) {
              if (codeName.startsWith("STATUS_") === true) {
                codeName = codeName.substring(7);
              }
            } else {
              codeName = response.status != null ? "error code: " + response.status : "unknown error code";
            }
            opName = response.operationId || response.operationType;
            extraText = strEmpty;
            switch (codeName) {
              ...
              case "VALIDATION_ERROR":
                ...
                break;
              case "FAILURE":
                if (response.httpResponseText != null) {
                ...
                break;
              default:
                message = ("Server returned " + codeName + " with no error message") + (opName != null ? " performing operation '" + opName + "'." : ".");
            }
          }
        }
      }
    ...
    "evalResult": function(request, response, results) {
      var evalResult, evalVars, origAutoDraw;
      if (isc.isA.Function(results.match)) {
        evalVars = request.evalVars;
        this.logDebug("evaling result" + (evalVars != null ? " with evalVars: " + (isc.Log.echo(evalVars)) : strEmpty));
  ...
  isc.defineClass("RPCManagerSS", isc.RPCManager).addClassProperties({
    ...
    "sendRequest": function(request, evalResult) {
      if ((request != null) && isc.isA.Object(request) === true) {
        request.useSimpleHttp = true;
        request.httpMethod = "POST";
        request.evalResult = evalResult == null ? true : evalResult;
        request.params = {
          isc_dataFormat: "JSON"
        };
        if (isc.isA.Object(request.data) === true) {
          request.data = isc.JSONSS.encode(request.data, {
            "prettyPrint": false
          });
        } else if (isc.isA.String(request.data) === false) {
          request.data = {};
        }
        this.Super("sendRequest", [request]);
      } else {
        this.Super("sendRequest", arguments);
      }
    },
    "loginRequired": function(reload, loginSuccessProcedure) {
      ...
      isc.RPCManagerSS.loginWindow = isc.LoginWindow.create({
        "loginSuccessProcedure": isc.isA.Function(loginSuccessProcedure) ? loginSuccessProcedure : void 0,
        "mainPageLogged": this.mainPageLogged,
        "reload": reload
      });
    },
    "logoutRequired": function() {
      var ref;
      ...
      if ((ref = simpleSyS.app.ViewLoaderC) != null) {
        if (typeof ref.setViewURL === "function") {
          ref.setViewURL(this.mainPageNotLogged);
        }
      }
    }
  });
