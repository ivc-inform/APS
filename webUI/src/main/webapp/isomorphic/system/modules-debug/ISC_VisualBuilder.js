...
/*
  ...
  Version v11.1p_2017-07-03/EVAL Development Only (2017-07-03)
...
,isc.A.rebindCurrentScreenDataSources=function isc_Project_rebindCurrentScreenDataSources(renames){
...
isc.A.dataSourceListDefaults={
    ...
    fields:[{
        ...
        formatCellValue:function(value,record,rowNum,colNum,grid){
            return grid.imgHTML(
                "[SKINIMG]/actions/download.png",null,null,null,null,grid.widgetImgDir);
        }
    }],
    ...
    dsContextMenuDefaults:{
        ...
        data:[{
            ...
            title:"Save seperately from screen..",
            enableIf:function(target,menu,item){
                return(menu._record.referenceInProject==false);
            },
            click:function(target,item,menu){
        ...
        {title:"Edit",enableIf:"node != null",
            click:function(target,item,menu){
                var selection=target.getSelection(),
                    ...
                    liveObject=editContext.getLiveObject(node)
                ;
                if(node){
                    if(liveObject&&isc.isA.DataSource(liveObject)){
                        if(menu.creator.isEmbeddedMockDataSource(liveObject.ID)){
                            var parentNode=editContext.getEditNodeTree().getParent(node);
                            if(parentNode){
                                if(parentNode.liveObject.editProxy.supportsInlineEdit){
                                    parentNode.liveObject.editProxy.startInlineEditing();
                                }
                            }
                        }else{
                            menu.creator.showDSEditor(liveObject.ID);
                        }
                    }else{
                        menu.creator.editComponent(node,liveObject);
                    }
                }
            }
        }
...
isc.A.projectComponentsDefaults={
            ...
            if(iscClass&&iscClass.isA(isc.DataSource)){
                if(newNode.loadData!=null&&!newNode.isLoaded){
                    ...
                    newNode.loadData(newNode,function(){
                        _this.addNode(newNode,parent,index,parentField,skipParentComponentAdd,dontShowFieldMapper);
                    });
                    return;
                }
    ...
    editorExit:function(event,record,newValue,rowNum,colNum){
        ...
        var oldValue=record[isc.DS.getAutoIdField(record)];
        if(oldValue==newValue){
            return;
        }
        var nodeProps={};
        nodeProps[isc.DS.getAutoIdField(record)]=newValue;
        this.setNodeProperties(record,nodeProps);
        var editorValues=this.creator.componentAttributeEditor.getValues();
        if(editorValues[isc.DS.getAutoIdField(record)]==oldValue){
            this.creator.componentAttributeEditor.setValue(isc.DS.getAutoIdField(record),newValue);
        }
    },
    canEditCell:function(rowNum,colNum){
        var record=this.getRecord(rowNum),
            fieldName=this.getFieldName(colNum);
        if(fieldName=="ID"&&record.type=="DataSource")
        {
            isc.say("ID cannot be changed in-place for DataSources");
            return false;
        }
...
isc.evalBoundary;isc.B.push(isc.A.updateReifyPreviewSoon=function isc_VisualBuilder_updateReifyPreviewSoon(){
    var builder=this;
...
,isc.A.loadBMMLMockup=function isc_VisualBuilder_loadBMMLMockup(mockupName,fileContent,dropMarkup,trimSpace,fillSpace,fieldNamingConvention,useDataSourceReferences){
                ...
                if(xmlContent){
                    ...
                    vb.projectComponents.getPaletteNodesFromXML(xmlContent,function(nodes){
                        ...
                        if(useDataSourceReferences){
                            var editNodes=vb.projectComponents.getEditContext().getEditNodeArray();
                            if(editNodes)editNodes.map(function(node){
                                if(node.isLoaded&&node.liveObject&&isc.isA.DataSource(node.liveObject)){
                                    var ds=node.liveObject;
                                    if(isc.isA.MockDataSource(ds)){
                                        ...
                                        var dsData=vb.projectComponents.editContext.makeDSPaletteNode(ds.name,dsType);
...
,isc.A.keyPress=function isc_VisualBuilder_keyPress(){
    if(isc.EH.getKey()=="Delete"){
        ...
        var selected=isc.SelectionOutline.getSelectedObject();
        if(selected&&selected.editContext){
            if(selected.editNode&&selected.editNode._canRemove===false){
                return;
            }
            selected.editContext.destroyNode(selected.editNode);
            return false;
...
,isc.A.init=function isc_VisualBuilder_init(){
    this.mockupsMenuDefaults={
        ...
        getSubmenu:function(item){
            ...
            if(item.title=="Switch Mockups"&&item.submenu.length==0){
                var builder=this.creator,
                    parentMenu=this,
            ...
            }else if(item.title=="Switch Projects"&&item.submenu.length==0){
                var builder=this.creator,
                    parentMenu=this,
                    ...
                    click=function(target,item,menu){
                        ...
                        parentMenu.confirmSaveProject(function(){
                            builder.loadProject(item.record.projectFileName);
                        });
                    },
                    checkIf=function(target,menu,item){
                ...
                menu.creator.confirmSaveScreen(function(){
                    isc.BMMLImportDialog.create({
                        ...
                        submit:function(fileName,outputFileName,fileContent,skin,dropMarkup,trimSpace,fillSpace,fieldNamingConvention){
                            menu.creator.loadBMMLMockup(fileName,fileContent,dropMarkup,trimSpace,fillSpace,fieldNamingConvention,false);
                            this.markForDestroy();
                        }
                    });
                });
    ...
    this.screenListDefaults={
        ...
        canEdit:true,
        modalEditing:true,
        recordDoubleClick:function(lg,record,recordNum,field,fieldNum,value,rawValue){
            this.edit(recordNum);
        },
        edit:function(recordNum){
            this.showField("fileName");
            this.startEditing(recordNum);
        },
        rowEditorExit:function(){
            this.delayCall("hideField",["fileName"],0);
        },
...
,isc.A.pageKeyPress=function isc_VisualBuilder_pageKeyPress(target,eventInfo){
    if(this.project.saveScreenDialog&&this.project.saveScreenDialog.containsFocus())return;
...
isc.evalBoundary;isc.B.push(isc.A._runProjectEnabled=function isc_VisualBuilder__runProjectEnabled(){
...
,isc.A.addDataSource=function isc_VisualBuilder_addDataSource(ds,fromWizard){
    ...
    if(this.dsEditorWindow||fromWizard){
        ...
        if(ds.serverType=="sql"||ds.serverType=="hibernate"){
            if(this.dsWizard!=null&&this.dsWizard.dsTypeRecord!=null){
                ...
                if(showDSImporter){
                    this.showDSImporter(ds.ID);
                }
            }
        }
        if(!fromWizard)renames=this.dsEditor.getFieldRenames();
    }
    ...
    this.project.rebindCurrentScreenDataSources(renames);
}
...
,isc.A.showDSImporter=function isc_VisualBuilder_showDSImporter(dsID){
    this.makeDSImporter();
    this.dsImporterWindow.show();
    this.dsImporter.getDefinedDataSources();
    if(dsID)this.dsImporter.selectDataSource(dsID);
}
,isc.A.makeDSImporter=function isc_VisualBuilder_makeDSImporter(){
    if(this.dsImporterWindow)return;
    var _this=this;
    if(!this.dsImporter){
        this.dsImporter=isc.DSImporter.create({
            width:"100%",
            height:"100%",
            autoDraw:false,
            selectDataSource:function(dsID){
                var data=this.dsList.data;
                if(!data||!data.lengthIsKnown()||data.getLength()==0){
                    this._selectedDataSource=dsID;
                }else{
                    var dsList=this.dsList;
                    dsList.setFilterEditorCriteria({dsName:dsID});
                    dsList.filterData(this.dsList.getFilterEditorCriteria(),function(){
                        var rowNum=data.findIndex("dsName",dsID);
                        if(rowNum>=0){
                            dsList.selectSingleRecord(rowNum);
                            dsList.scrollToRow(rowNum);
                        }
                    });
                }
            },
            dsListProperties:{
                dataArrived:function(){
                    if(this.creator._selectedDataSource){
                        var dsID=this.creator._selectedDataSource;
                        this.creator._selectedDataSource=null;
                        this.creator.delayCall("selectDataSource",[dsID]);
                    }
                }
            },
            dsActionsProperties:{
                initWidget:function(){
                    this.buttons.add(isc.LayoutSpacer.create());
                    this.buttons.add({title:"Close",width:100,click:function(){
                        _this.dsImporterWindow.hide();
                    }});
                    this.Super("initWidget",arguments);
                }
            }
...
isc.evalBoundary;isc.B.push(isc.A.getSchemaDataSourceIDs=function isc_VisualBuilder_getSchemaDataSourceIDs(operation,serviceName,serviceNamespace){
    var schemaDSIDs={};
...
isc.evalBoundary;isc.B.push(isc.A.getControlHeightUsingItsParents=function isc_MockupImporter_getControlHeightUsingItsParents(layout,control){
