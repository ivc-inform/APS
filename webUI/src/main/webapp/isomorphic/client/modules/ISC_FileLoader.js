...
isc.version = "v11.1p_2017-07-03/EVAL Development Only";
isc.versionNumber = "v11.1p_2017-07-03";
isc.buildDate = "2017-07-03";
isc.expirationDate = "2017.09.01_04.55.13";
...
useCSSLoaders:!isc.SA_Page.isLoaded()&&
               ...
               ((isc.Browser.isSafari&&!isc.Browser.isChrome&&isc.Browser.version<7))),
...
loadISC:function(skin,modules,onload){
    this.loadModules(modules||this.defaultModules);
    this.loadSkin(skin||this.defaultSkin,onload);
},
loadSkin:function(skin,onload){
    ...
    this._queueFiles(skinDir+"load_skin.js",onload,"js",{defer:true,addVersionToLoadTags:true});
},
...
fileLoaded:function(fileID,fileContents,ignoreThisArg,delayed){
    ...
    if(fileContents!=null&&fileContents.status==404){
        var tempConfig=this._fileConfig[fileID];
        if(tempConfig!=null&&tempConfig.loadFailedPrompt){
            isc.logWarn("FileLoader: "+tempConfig.loadFailedPrompt);
...
showThrobber:function(message,style,image,width,height){
    ...
    throbber.style.left="45%";
    throbber.style.top="40%";
    throbber.style.height="auto";
    throbber.style.padding=this.throbberPadding+"px";
    ...
    throbber.appendChild(img);
    if(message){
        var text=document.createTextNode(message);
        var messageSpan=document.createElement("SPAN");
        messageSpan.className=style;
        messageSpan.appendChild(text);
        throbber.appendChild(messageSpan);
    }
